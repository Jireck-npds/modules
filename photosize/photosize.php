...
//on définit le répertoire dans lequel on souhaite travailler
   ...
   include ('header.php');
   include ("modules/$ModPath/lang/photosize-$language.php");
   
   if (autorisation(1) or $admin) {
   echo '<div class="card">';
   echo '<div class="card-block"><h4 class="card-title">'.phot_translate("$mNom").'</h4>';
...
//si le fichier n'est pas un répertoire
   ...
   {
   $messagefjpg = '<p class="text-danger"><i class="fa fa-info-circle" aria-hidden="true"></i> '.phot_translate("Le fichier doit être impérativement une image au format jpg").'</p>';
   }
   else
   {
// Copie dans le repertoire du script avec un nom incluant l'heure à la seconde près
   ...
   {
   echo '<p class="lead text-info"><i class="fa fa-info-circle" aria-hidden="true"></i> '.phot_translate("L'opération s'est bien passée").'</p>';
   }
   else
   {
   echo '<p class="lead text-danger">'.phot_translate("Le fichier n'a pas été uploadé, trop volumineux").'</p>';
   }
   }
...
echo '<form enctype="multipart/form-data" action="modules.php?ModPath='.$ModPath.'&amp;ModStart=photosize" method="post">
         ...
         <fieldset class="form-group has-success">
            ...
            <li>'.phot_translate("Maxi = largeur 900 pixels").'</li>
            <li>'.phot_translate("Normal = largeur 600 pixels").'</li>
            <li>'.phot_translate("Mini = largeur 400 pixels").'</li>
            </ul></p>
         </fieldset>
         <fieldset class="form-group">
            <button type="submit" class="btn btn-outline-primary btn-sm"><i class="fa fa-check" aria-hidden="true"></i> '.phot_translate("Redimensionner").'</button>
         </fieldset>
      </form>';
   ...
   echo '<div class="alert alert-info"><p>'.phot_translate("Clic droit sur l'image puis enregistrer l'image sous").'...<br />'.phot_translate("Enregistrer sur votre bureau sans changer le nom du fichier qui est spécialement codifié").'.</p>';
   echo '<p>'.phot_translate("Retournez sur la page de saisie de votre publication").'.</p>';

   echo '<p>'.phot_translate("Placer ensuite le curseur à l'endroit où vous voulez mettre la photo puis cliquer sur l'icone téléchargement").' <img src="editeur/tinymce/plugins/npds/images/npds_upload.png" width="20px" /> '.phot_translate("une fenêtre s'ouvrira où vous sélectionnerez le fichier photo préparée puis cliquez sur le bouton joindre").'.</p></div>';
   }
   echo '</div></div>';
   }
   else {
