...
//==>traitement des mots clefs
    ...
    yt.motclefs_list = yt.keyword_vid.split(expreg);
    var keyword_link=[];
...
//<==traitement des mots clefs
    ...
    html.push('<li class="youtubebox" onclick="loadVideo(\'', playerUrl, '\', true,'+i+');show_visua();">',
...
//  var iem = document.getElementById('yt_video_chan_s');//ajoute une liste de thumbnail à entete
//  iem.innerHTML = html.join('');//ajoute une liste de thumbnail à entete

//   if (entries.length > 0) {
//     loadVideo(entries[0].media$group.media$content[0].url, false,0,0);
//   }//affiche la premiere video de la serie de navigation
}
function showMychanel(data) {
  var entries = data.entry
  yt.nbvi_chanel = entries.gd$feedLink[5].countHint;
