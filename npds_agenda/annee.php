...
function calend($an, $month) {
   ...
   echo '
   <p class="text-center"><a class="btn btn-secondary btn-sm" href="modules.php?ModPath='.$ModPath.'&amp;ModStart=calendrier&amp;month='.$month.'&an='.$an.'">'.$mois_en_clair.'</a></p>
   ...
   for ($i = 1; $i < 8; $i++) {
      ...
      if ($i < $premier_jour)
         ...
         if($tab_jours[$ce_jour]) {
            /*Si jour ferie sans evenement*/
            if (!array_key_exists($ce_jour,$afftitre) and $fetetitre[$ce_jour] != '') $cla = 'table-warning';
      ...
      for ($i = 1; $i < 8; $i++) {
         if($jour_suiv > $dernier_jour)
            ...
            if($tab_jours[$jour_suiv]) {
               /*Si jour ferie sans evenement*/
               if ($afftitre[$jour_suiv] == '' and $fetetitre[$jour_suiv] != '') $cla = 'table-warning';
               else if ($afftitre[$jour_suiv] != '') $cla = 'table-info';
               else if ($afftitre[$jour_suiv] != '' and $fetetitre[$jour_suiv] != '') $cla = 'table-info';
               /*Ajoute le jour et reste sur la meme page + css jour evenement*/
               echo  '<td class="text-center '.$cla.'"><a class="" href="modules.php?ModPath='.$ModPath.'&amp;ModStart=calendrier&amp;subop=jour&amp;date='.$date.'" data-toggle="tooltip" data-placement="bottom" data-html="true" title="';
               if(isset($fetetitre) and array_key_exists($jour_suiv,$fetetitre))
                  echo aff_langue($fetetitre[$jour_suiv]);
               if(array_key_exists($jour_suiv,$afftitre))
                  echo $afftitre[$jour_suiv];
//               .aff_langue($fetetitre[$jour_suiv]).''.$afftitre[$jour_suiv].
               
               echo '"><span class="'.$cs.'">'.$jour_suiv.'</span></a>
               </td>';
            }
            else
               //css libre
