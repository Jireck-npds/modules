...
//Accès direct à un sujet
	...
	<option>'.aff_langue('Accès sujets disponibles').'</option>';
...
/*Ordre par defaut*/
   ...
   echo '<p>'.ag_translate('Trier par').'
	...
	<th class="text-center">'.ag_translate('Sujet').'</th>
...
/*Fin securite*/
	...
	if ($ok)
	{
		...
		echo '<p class="lead text-center"><i class="fa fa-info-circle" aria-hidden="true"></i> '.ag_translate('Cet évènement est maintenant effacé.').'</p>
		<div class="text-center"><a class="btn btn-outline-primary btn-sm" href="'.$ThisFile.'">'.ag_translate('Retour').'</a></div>';
	}
	else
	{
...
/*Verif id - auteur*/
		...
		if ($tot != 0)
		{
			echo '<p class="lead text-center">'.ag_translate('Etes-vous certain de vouloir supprimer cet évènement ?').'</p>
			<div class="text-center"><a class="btn btn-outline-primary btn-sm mr-2" href="'.$ThisFile.'">'.ag_translate('NON').'</a>
...
<!--<li><span class="text-danger">*</span> '.ag_translate('Champ obligatoire').'</li>-->
    ...
    <input type="hidden" name="debut" value="'.$debut.'" />'.ag_translate('Jours séléctionnés').' :';
	...
	<label class="mr-sm-2" for=""><strong>'.ag_translate('Sujet').'&nbsp;<span class="text-danger">*</span></strong></label>
...
/*Requete liste categorie*/
	...
	<div class="text-center"><a class="btn btn-secondary btn-sm float-right" href="javascript:history.back()">'.ag_translate('Retour').'</a></div>';
	echo '<div></div>';	
}
...
function validedit ($id, $debut, $topicid, $titre, $desc, $longdesc, $lieu)
{
	...
	{
		echo '<p class="lead text-center"><i class="fa fa-info-circle" aria-hidden="true"></i> '.ag_translate('Vous n\'avez pas remplis les champs obligatoires').'</p>
		<div class=""><i class="fa fa-info-circle" aria-hidden="true"></i><a href="javascript:history.back()"> '.ag_translate('Retour').'</a></div>';
	}
	else
	{
		...
		if ($query)
		{
			...
			{
				$subject = ag_translate('Modification évènement pour agenda');
				$message = ag_translate('Un évènement modifié est à valider pour l\'agenda.');
				send_email($receveur,$subject, $message, "", true, "html");
			}
			if ($revalid == 3)
			{
				echo '<p class="lead text-center"><i class="fa fa-info-circle mr-2" aria-hidden="true"></i>'.ag_translate('Un administrateur validera vos changements rapidement').'</p';
			}
			else if ($revalid == 1)
			{
				echo '<p class="lead text-center"><i class="fa fa-info-circle mr-2" aria-hidden="true"></i>'.ag_translate('Vos changements ont bien été ajoutés à l\'agenda').'</p>';
			}
		}
	}
